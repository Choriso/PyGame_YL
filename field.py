...
class Field:
    def __init__(self, size, tilemap, bg_color):
        ...
        self.cells = [[Cell(x * self.cell_size, y * self.cell_size, tilemap.subsurface(pygame.Rect(random.randrange(0, tilemap.get_width(), 16), random.randrange(0, tilemap.get_height(), 16), 16, 16)), self.cell_size) for x in range(size)] for y in range(size)]
    ...
    def draw_dashed_line(self, screen):
        ...
        dash_length = 10
        space_length = 5
        for x in range(self.cell_size, self.size * self.cell_size, self.cell_size):
            for y in range(0, self.size * self.cell_size, dash_length + space_length):
                pygame.draw.line(screen, (255, 255, 255), (x + self.left, y + self.top), (x + self.left, y + dash_length + self.top))
                pygame.draw.line(screen, (255, 255, 255), (y + self.left, x + self.top), (y + dash_length + self.left, x + self.top))
...
def main():
    ...
    tilemap = pygame.image.load('TexturedGrass.png')  # Загрузите ваш tilemap здесь
